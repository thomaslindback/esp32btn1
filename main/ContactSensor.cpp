...
static const char * CS_TAG = "ContactSensor.cpp";

extern ContactSensor gContactSensor;

ContactSensor::ContactSensor() {}

ContactSensor::ContactSensor(gpio_num_t gpioNum) {
    mGPIONum = gpioNum;
}
...
void IRAM_ATTR contact_sensor_isr_handler(void * arg)
{
    ...
    xTimerStartFromISR(gContactSensor.mContactSensorTimer,
                       &taskWoken); // If the timer had already been started ,restart it will reset its expiry time
}

esp_err_t ContactSensor::Init() {
    return Init(mGPIONum);
}

esp_err_t ContactSensor::Init(gpio_num_t gpioNum)
{
    ...
    ret = gpio_isr_handler_add(gpioNum, contact_sensor_isr_handler, (void *) gpioNum);
    ESP_RETURN_ON_ERROR(ret, CS_TAG, "gpio_isr_handler_add failed: %s", esp_err_to_name(ret));

    mContactSensorTimer = xTimerCreate("CsTmr",               // Just a text name, not used by the RTOS kernel
...
void ContactSensor::TimerCallback(TimerHandle_t xTimer)
{
    ...
    ESP_LOGI(CS_TAG, "ContactSensor::TimerCallback %lu", gpio_num);
}
